...
export default function SideNav() {
  ...
  return (
    <nav className="w-full flex flex-col gap-4">
      ...
      <div className="w-fit flex flex-row sm:flex-col items-start gap-4">
        {navLinks.map((l, key) => (
          <Link
            key={key}
            aria-selected={l.href === router.pathname}
            className="w-fit text-sm sm:text-lg font-serif text-[#6f6f6f] hover:text-white transition-all ease-linear px-3 py-2 aria-selected:bg-[#282626] rounded-lg aria-selected:text-white"
            href={l.href}
          >
            {l.title.toLowerCase()}
          </Link>
